classdef CLASS_optogenetics_legacy_obj < handle
	% 	
	% 	GITHUB HSOM VERSION
	% 
	...
	% 
	% 	VERSION CODE: ['CLASS_optogenetics_legacy_obj v2.0 Modified 10-29-19 18:36 | obj created: ' datestr(now)];
	% 
	% 	VERSION 1.x
	...
	methods
		...
		%-------------------------------------------------------
		function obj = CLASS_optogenetics_legacy_obj(Mode, stim_protocol, stimtype, ChR2data, Xdata, collateKey)
			% 
			...
			% 							obj = CLASS_optogenetics_legacy_obj('collate', 'A', 'A', [], [], 'ecdf')
			% 
			% 	stim_protocols:
            ...
            %   MELIE USE:
			%       obj = CLASS_optogenetics_legacy_obj('batch', 'A', 'A', [], [], 'emery')
			%	obj = CLASS_optogenetics_legacy_obj('collate', 'A', 'A', [], [], 'bootAUC')
			% 
			% 	obj = CLASS_optogenetics_legacy_obj('collate', 'nt', 'NaiveTest-No Juice', [], [], 'ntRaster')
			% 	obj = CLASS_optogenetics_legacy_obj('collate', 'ntj', 'NaiveTest+Juice', [], [], 'ntjRaster')
			% 	obj = CLASS_optogenetics_legacy_obj('batch', 'ntj', 'NaiveTest+Juice', [], [], 'ntjRaster')
			% 
			obj.iv.runID = randi(10000);
			obj.iv.versionCode = ['CLASS_optogenetics_legacy_obj v2.0 Modified 10-29-19 18:36 | obj created: ' datestr(now)];
		...
		function getDataset(obj, ChR2data, Xdata)
			...
			if strcmpi(obj.iv.Mode, 'batch')
		     ...
		     %                eval(['sObj = sObj.' sObjfield{1} ';']);
							...
							% 
							CLASS_optogenetics_legacy_obj('default', obj.iv.stim_protocol, obj.iv.stimtype);
						end	
						disp(' ')
				    ...
				    for ufile = 1:ifile-1
				    	...
				    	CLASS_optogenetics_legacy_obj('default', obj.iv.stim_protocol, obj.iv.stimtype, file_paths(ufile).ChR2, file_paths(ufile).X);
				    	disp('	File saved and complete.')
				    	disp('-----------------------------------------------------------------')
			    	end
			    	cd(returndir);
