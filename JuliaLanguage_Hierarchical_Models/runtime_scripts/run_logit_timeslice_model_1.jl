# run_logit_timeslice_model_ELAPSEDTIME_2.jl

# USER MUST SET THESE:
collatedPath = "/Users/lilis/Dropbox (MIT)/1 ASSAD LAB/testFiles/testHOST/sessionHOST/CSVfigures/collatedCSV_b5d13only"
...
postprocessingfunction = bootlogit_timeslice_postprocessingfunction1

warning("suppressing figures from individual sessions, but they will still be saved to host folders for that session.")
println("	")
(fails, results) = run_collated_model(collatedPath, modelpackagefunction; pathIDx = [1,2],runFails=false, failDirs=[], postprocessingfunction=postprocessingfunction,
